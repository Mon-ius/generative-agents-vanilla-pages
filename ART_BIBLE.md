...
> **Scope & honesty note (read first).** All art in this project is generated by
> code (`tools/gen_assets.mjs` paints PNG sprites; the world is composited as a
> tilemap). There is no image-generation tool in this pipeline, and the original
> "Smallville" tileset (Cute RPG World) is licensed and cannot be redistributed.
> So the bar here is **polished, cohesive, layered procedural pixel art** — a real
> production step above flat blocks — not a commissioned AAA pack. The path to the
> final AAA tier is to drop in a permissively-licensed (CC0) art pack or commission
> art; the renderer is asset-driven, so swapping the PNGs in `assets/sprites/` is
> all that's required. See "Upgrade path".
