...
- `test/smoke.node.mjs` — headless harness for the DOM-free core (world integrity, **determinism**, save/load, pathfinding, renderer re-route wall-legality, group-conversation invariants). 24 checks.
...
```bash
node test/smoke.node.mjs    # or: npm run smoke   — 24 checks, exits non-zero on failure
...
```
The smoke harness is a **single fast file with no per-test filter** — it runs all 24 checks and
...
losing all 88 streets or all 274 courtyards still passes 24/24; only the duplicate-coord and
...
window.runSmokeTest()       // browser console; a SEPARATE smaller suite defined in main.js (NOT the
                            // 24-check node harness); MUTATES the live sim (steps/selects/saves/resets);
