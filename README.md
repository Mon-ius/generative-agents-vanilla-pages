...
/
  ...
  /assets/
    manifest.json         # sprite manifest (tile size + names/dims)
    /sprites/             # original pixel-art PNG tiles & objects (generated)
  /tools/
    gen_assets.mjs        # dev-only, dependency-free PNG sprite generator (NOT deployed)
  /css/
  ...
  /js/
    ...
    assets.js             # loads the sprite PNGs into Image objects (node-safe)
    /simulation/
...
- The town is composited from **original pixel-art sprite PNGs** in
  `assets/sprites/` (terrain tiles, walls/floors, furniture, trees), generated by
  the dependency-free `tools/gen_assets.mjs`. If the sprites fail to load the
  renderer falls back to drawing the town procedurally, so it always works.
